...
SensorFusion::SensorFusion(drivers::ImuDriver& imu, drivers::BarometerDriver& baro)
    : imuDriver(imu), baroDriver(baro), lastAltitude(0.0f), lastUpdateTime(std::chrono::system_clock::now()) {
...
FusedSensorData SensorFusion::updateAndPublish() {
    ...
    fusedData.timestamp_us = esp_timer_get_time(); // Use ESP timer for microsecond precision

    lastAltitude = currentAltitude;
    lastUpdateTime = currentTime;

    // Publish fused data
    MREQ_PUBLISH(fused_sensor_data, fusedData);
